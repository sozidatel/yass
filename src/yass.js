const VERSION = __YASS_VERSION__;
const SELECTOR = 'select[data-yass]';
const STYLE_SELECTOR = 'style[data-yass-styles]';
...
const documentObject = host.document || null;
const librarySymbol = typeof Symbol === 'function' ? Symbol.for('yass.library') : null;
const storedApi = librarySymbol ? host[librarySymbol] || null : null;
const existingApi = storedApi?.version === VERSION ? storedApi : null;
const embeddedStyles = typeof __YASS_BASE_CSS__ === 'string'
    ? __YASS_BASE_CSS__
    : '';
...
let defaults = mergeOptions(builtInDefaults, host.YassConfig || {});
let serial = 0;
...
let positionFrame = 0;
let activeConnectionObserver = null;
...
function mergeOptions(base, overrides) {
    ...
    return {
        ...
        openOnFocus: source.openOnFocus === undefined ? base.openOnFocus : Boolean(source.openOnFocus),
        keyboardLayout: source.keyboardLayout === undefined ? base.keyboardLayout : Boolean(source.keyboardLayout),
        injectStyles: source.injectStyles === undefined ? base.injectStyles : Boolean(source.injectStyles),
        nonce: source.nonce === undefined ? base.nonce : source.nonce,
    };
}
...
function optionsFor(select, options) {
    ...
    if (dataset.yassPlaceholder !== undefined) {
        resolved.messages.searchPlaceholder = dataset.yassPlaceholder;
    }
    if (dataset.yassEmpty !== undefined) {
...
function optionEntry(option, depthOffset) {
    ...
    const parentDisabled = option.parentElement?.tagName === 'OPTGROUP'
...
function labelledByElements(select) {
    ...
    return ids
        .map((id) => select.ownerDocument.getElementById(id))
        .filter(Boolean);
}

function labelledByText(select) {
    return labelledByElements(select)
...
function moveActive(direction) {
    ...
    const target = current === -1
        ...
        : Math.max(0, Math.min(indexes.length - 1, current + direction));
...
function openInstance(instance) {
    if (isEffectivelyDisabled(instance.select)
        || instance.select.hidden
        || !instance.trigger.isConnected
        || instance.root.getClientRects().length === 0) {
        return;
    }
...
class YassInstance {
    constructor(select, options) {
        ...
        this.navigableIndexSet = new Set();
        ...
        this.labels.forEach((label) => {
            if (this.implicitLabels.includes(label)
                || (select.id && label.getAttribute('for') === select.id)) {
        ...
        this.onInvalid = () => {
            ...
            openInstance(this);
            // Native constraint validation may move focus after the `invalid`
            // listener returns. Re-assert the useful correction target once
            // that browser step has completed.
            host.setTimeout(() => {
...
if (documentObject && !existingApi) {
    documentObject.addEventListener('pointerdown', (event) => {
        if (activeInstance
            && !activeInstance.root.contains(event.target)
...
const api = existingApi || localApi;
...
const publicEnhance = api.enhance;
const publicEnhanceAll = api.enhanceAll;
const publicGet = api.get;
const publicConfigure = api.configure;
const publicInjectStyles = api.injectStyles;
